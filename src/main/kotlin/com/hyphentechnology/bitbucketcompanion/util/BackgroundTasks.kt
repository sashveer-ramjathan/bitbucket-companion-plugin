...
object BackgroundTasks {
     ...
     * notifying the user) if workspace/email/token aren't fully configured yet.
     */
    fun buildApiClient(project: Project?): BitbucketApiClient? {
        ...
        if (state.workspace.isBlank() || state.email.isBlank() || token.isNullOrBlank()) {
            notifyError(project, "Bitbucket Companion isn't configured yet - set workspace/email/token in Settings > Tools > Bitbucket Companion.")
            return null
        }
        return BitbucketApiClient(state.workspace, state.email, token)
    }
